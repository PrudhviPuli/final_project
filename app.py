...
def getUnreadMessages():
    ...
    if request.method == 'GET':
        ...
        msgs = query_db('select count(m.channel_id) numb, channel_id from messages m where m.replies_to = 0 and m.channel_id in '+ 
                        '(select distinct channel_id from messages except select distinct channel_id from joint_unread) ' + 
                        'group by m.channel_id union select count(m.channel_id) numb, m.channel_id from messages m, joint_unread ' +
...
def updateUnreadMessages():
    user = authenticateUser(request)
    if not user:
        return app.send_static_file('404.html'), 401
    if request.method == 'POST':
        d = query_db('delete from joint_unread where user_id = ? and channel_id = ?', (request.headers['User-Id'], request.args['channel_id']), one=True)
        u = query_db('insert into joint_unread select ?, ?, max(id) from messages where channel_id = ?', (request.headers['User-Id'], request.args['channel_id'],  request.args['channel_id']), one=True)
        return {'status': 'Success'}, 200
